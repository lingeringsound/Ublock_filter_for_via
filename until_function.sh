...
local file="${1}"
test ! -f "${file}" && return
local lite_content="$(cat ${file} | grep -Ev '#\@\?#|\$\@\$|#\%#|#\@\%#|#\@\$\?#|#\$\?#|#\$#|#\?#|#@?#\+js\(|#\%#\/\/scriptlet|##\^|redirect=|removeparam=|\,replace=|redirect-rule=|\$removeparam|\$badfilter|\$empty|\$generichide|\$match-case|\$object|\$object-subrequest|\$~badfilter|\$~empty|\$~generichide|\$~removeparam|\$~match-case|\$~object|\$~object-subrequest|\,badfilter$|\,badfilter\,|\,empty$|\,empty\,|\,generichide$|\,generichide\,|\,match-case$|\,match-case\,|\,object$|\,object-subrequest$|\,object-subrequest\,|\,object\,|\,~badfilter$|\,~badfilter\,|\,~empty$|\,~empty\,|\,~generichide$|\,~generichide\,|\,~match-case$|\,~match-case\,|\,~object$|\,~object-subrequest$|\,~object-subrequest\,|\,~object\,|\$csp|\,csp=|\,denyallow=|permissions=|\:(matches-path|-abp-contains|-abp-properties|contains|has-text|matches-css|matches-css-before|matches-css-after|xpath|nth-ancestor|upward|remove|style|watch-attr)|\$replace=|\$urlskip=|\,urlskip=|\$uritransform=|\,uritransform=|to=|^/(\^|\\|\[|\(\?)|^\*\$|\$cname|\$frame|\$ghide|\$media|\$ping|\$popunder|\$~cname|\$~frame|\$~ghide|\$~media|\$~ping|\$~popunder|\,cname$|\,cname\,|\,frame$|\,frame\,|\,ghide$|\,ghide\,|\,media$|\,media\,|\,ping$|\,ping\,|\,popunder$|\,popunder\,|\,~cname$|\,~cname\,|\,~frame$|\,~frame\,|\,~ghide$|\,~ghide\,|\,~media$|\,~media\,|\,~ping$|\,~ping\,|\,~popunder$|\,~popunder\,|:matches-attr|:matches-property|:min-text-length|:remove\(\)|:others\(|:shadow\(' | busybox sed -e '/^\!/d;/^[[:space:]]*$/d' \
